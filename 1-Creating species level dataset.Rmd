...
*NOTE* This code demonstrates how the level 2 data is generated. This will not export the data file. The function updateIDs() is commented here as to not generate new ID numbers and mess up the master_id_list file since the level 2 data file is already generated.
...
traits.lvl1 <- read.csv(paste0(infol,"trait_dataset_level1-2023-06-28.csv"))
```
...
trait.DVM <- trait.cat %>% 
  ...
  mutate(DVM.absent = if_else(grepl("absent", traitValue)==TRUE,1,0),
         DVM.present = if_else(grepl("absent", traitValue)==TRUE,1,0),
