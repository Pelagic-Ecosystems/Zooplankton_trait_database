...
library(openxlsx)
library(lubridate)
library(ggpubr) # for regression line in ggplot
library(lmodel2)
library(ggnewscale) # for having multiple color scales in ggplot
library(cowplot)
library(phylolm) # for regressions with phylogenetic effects
library(Rphylopars) # for phylogenetic imputation
...
for(i in c(1:15)){
  myplots[[i]] <-  ggplot(filter(traits.lvl2.sub, traitX == var.table[i,3] & traitY == var.table[i,4]),
...
# 3. Merge both figures into one
fig <- plot_grid(g1, g2, nrow = 1)
fig

ggsave("figures/fig_8_comparison_estimation_percent_removed.png",
       width = 21, height = 23, units = "cm", dpi = 600)
#TODO: manually anotate legend
```
